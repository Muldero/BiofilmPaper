...
k_data = read.csv("Plot_Code/Plot_Data/K_Synch_Data_0.2.csv",
                  header = TRUE)
glu_data = read.csv("Plot_Code/Plot_Data/Glu_Synch_Data_0.2.csv",
                    header = TRUE)

### Figure 6A: Glutamate synchronization
...
p1_osc <- glu_data %>% ggplot(aes(x = time)) +
  ...
  geom_line(aes(y = g_osc.3, lty = "Signaling Fraction"),
            color = "#F8766D", alpha = 0.7) +
  geom_line(aes(y = g_osc.4, lty = "Signaling Fraction"),
            color = "#F8766D", alpha = 0.7) +
...
### Figure 6B: Potassium synchronization
...
ggsave("Plot_Code/Final_Plots/Figure6_Synchronization.tiff", combined_plot,
